...
contract AnonSocial is Ownable {
    ...
    function buyShares(address sharesSubject, uint256 amount, address referrer) public payable {
        ...
        if(user.referrer == address(0)){
            if(users[referrer].exists){
                ...
                user.referrer = protocolFeeDestination;
            }
        }
        ...
        (bool success1, ) = sharesSubject.call{value: subjectFee}("");

        users[user.referrer].referralBonus += protocolFee;
        ...
        emit ReferrerPaid(msg.sender, user.referrer, protocolFee);
    }
